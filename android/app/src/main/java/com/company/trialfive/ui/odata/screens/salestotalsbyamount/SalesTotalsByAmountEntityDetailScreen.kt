package com.company.trialfive.ui.odata.screens.salestotalsbyamount
...
import com.sap.cloud.android.odata.northwindentities.SalesTotalsByAmount

val SalesTotalsByAmountEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SalesTotalsByAmount.companyName.name,
                                value = entity.getOptionalValue(SalesTotalsByAmount.companyName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SalesTotalsByAmount.orderID.name,
                                value = entity.getOptionalValue(SalesTotalsByAmount.orderID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SalesTotalsByAmount.saleAmount.name,
                                value = entity.getOptionalValue(SalesTotalsByAmount.saleAmount)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SalesTotalsByAmount.shippedDate.name,
                                value = entity.getOptionalValue(SalesTotalsByAmount.shippedDate)?.toString() ?: ""
                            )
                        )
                }
            }

...
val SalesTotalsByAmountEntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        SalesTotalsByAmountEntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        SalesTotalsByAmountEntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        SalesTotalsByAmountBlankScreen(viewModel)
                    }
                }
            }
        }
...
val SalesTotalsByAmountBlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

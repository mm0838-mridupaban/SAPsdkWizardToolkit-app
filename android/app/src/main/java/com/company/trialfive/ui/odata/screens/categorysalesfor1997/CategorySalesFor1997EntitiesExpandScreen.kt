...
val CategorySalesFor1997EntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        CategorySalesFor1997EntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        CategorySalesFor1997EntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        CategorySalesFor1997BlankScreen(viewModel)
                    }
                }
            }
        }
...
val CategorySalesFor1997BlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

package com.company.trialfive.ui.odata.screens.shipper
...
import com.sap.cloud.android.odata.northwindentities.Shipper

val ShipperEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Shipper.shipperID.name,
                                value = entity.getOptionalValue(Shipper.shipperID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Shipper.companyName.name,
                                value = entity.getOptionalValue(Shipper.companyName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Shipper.phone.name,
                                value = entity.getOptionalValue(Shipper.phone)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, Shipper.orders as NavigationProperty)
                    }) {

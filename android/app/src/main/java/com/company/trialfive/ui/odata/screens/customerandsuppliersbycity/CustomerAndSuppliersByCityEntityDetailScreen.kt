package com.company.trialfive.ui.odata.screens.customerandsuppliersbycity
...
import com.sap.cloud.android.odata.northwindentities.CustomerAndSuppliersByCity

val CustomerAndSuppliersByCityEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = CustomerAndSuppliersByCity.companyName.name,
                                value = entity.getOptionalValue(CustomerAndSuppliersByCity.companyName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = CustomerAndSuppliersByCity.relationship.name,
                                value = entity.getOptionalValue(CustomerAndSuppliersByCity.relationship)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = CustomerAndSuppliersByCity.city.name,
                                value = entity.getOptionalValue(CustomerAndSuppliersByCity.city)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = CustomerAndSuppliersByCity.contactName.name,
                                value = entity.getOptionalValue(CustomerAndSuppliersByCity.contactName)?.toString() ?: ""
                            )
                        )
                }
            }

...
val ProductSalesFor1997EntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        ProductSalesFor1997EntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        ProductSalesFor1997EntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        ProductSalesFor1997BlankScreen(viewModel)
                    }
                }
            }
        }
...
val ProductSalesFor1997BlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

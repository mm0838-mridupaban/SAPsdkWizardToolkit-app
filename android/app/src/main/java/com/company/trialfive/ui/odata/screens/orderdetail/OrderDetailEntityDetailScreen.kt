package com.company.trialfive.ui.odata.screens.orderdetail
...
import com.sap.cloud.android.odata.northwindentities.OrderDetail

val OrderDetailEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetail.orderID.name,
                                value = entity.getOptionalValue(OrderDetail.orderID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetail.productID.name,
                                value = entity.getOptionalValue(OrderDetail.productID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetail.unitPrice.name,
                                value = entity.getOptionalValue(OrderDetail.unitPrice)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetail.quantity.name,
                                value = entity.getOptionalValue(OrderDetail.quantity)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetail.discount.name,
                                value = entity.getOptionalValue(OrderDetail.discount)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, OrderDetail.order as NavigationProperty)
                    }) {
                        Text("Order", color = MaterialTheme.fioriHorizonAttributes.SapFioriColorT6)
                    }

                    TextButton(onClick = {
                        onNavigateProperty(entity, OrderDetail.product as NavigationProperty)
                    }) {

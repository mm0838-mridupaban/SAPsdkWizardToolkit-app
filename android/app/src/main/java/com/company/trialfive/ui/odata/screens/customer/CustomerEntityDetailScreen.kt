package com.company.trialfive.ui.odata.screens.customer
...
import com.sap.cloud.android.odata.northwindentities.Customer
        ...
        Column {
            ...
            if (entity != null) {
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Customer.companyName.name,
                                value = entity.getOptionalValue(Customer.companyName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Customer.contactName.name,
                                value = entity.getOptionalValue(Customer.contactName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Customer.contactTitle.name,
                                value = entity.getOptionalValue(Customer.contactTitle)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Customer.address.name,
                                value = entity.getOptionalValue(Customer.address)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Customer.phone.name,
                                value = entity.getOptionalValue(Customer.phone)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Customer.fax.name,
                                value = entity.getOptionalValue(Customer.fax)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, Customer.orders as NavigationProperty)
                    }) {
                        Text("Orders", color = MaterialTheme.fioriHorizonAttributes.SapFioriColorT6)
                    }

                    TextButton(onClick = {
                        onNavigateProperty(entity, Customer.customerDemographics as NavigationProperty)
                    }) {
                        Text("CustomerDemographics", color = MaterialTheme.fioriHorizonAttributes.SapFioriColorT6)
                    }

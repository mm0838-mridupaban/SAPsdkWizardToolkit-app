package com.company.trialfive.ui.odata.screens.region
...
import com.sap.cloud.android.odata.northwindentities.Region

val RegionEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Region.regionID.name,
                                value = entity.getOptionalValue(Region.regionID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Region.regionDescription.name,
                                value = entity.getOptionalValue(Region.regionDescription)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, Region.territories as NavigationProperty)
                    }) {

package com.company.trialfive.ui.odata.screens.orderdetailsextended
...
import com.sap.cloud.android.odata.northwindentities.OrderDetailsExtended

val OrderDetailsExtendedEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.discount.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.discount)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.orderID.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.orderID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.productID.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.productID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.productName.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.productName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.quantity.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.quantity)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.unitPrice.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.unitPrice)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = OrderDetailsExtended.extendedPrice.name,
                                value = entity.getOptionalValue(OrderDetailsExtended.extendedPrice)?.toString() ?: ""
                            )
                        )
                }
            }

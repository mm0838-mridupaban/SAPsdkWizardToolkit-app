...
val OrderDetailsExtendedEntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        OrderDetailsExtendedEntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        OrderDetailsExtendedEntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        OrderDetailsExtendedBlankScreen(viewModel)
                    }
                }
            }
        }
...
val OrderDetailsExtendedBlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

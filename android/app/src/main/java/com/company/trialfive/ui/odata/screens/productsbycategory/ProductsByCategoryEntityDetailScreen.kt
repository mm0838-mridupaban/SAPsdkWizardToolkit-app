package com.company.trialfive.ui.odata.screens.productsbycategory
...
import com.sap.cloud.android.odata.northwindentities.ProductsByCategory

val ProductsByCategoryEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = ProductsByCategory.categoryName.name,
                                value = entity.getOptionalValue(ProductsByCategory.categoryName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = ProductsByCategory.discontinued.name,
                                value = entity.getOptionalValue(ProductsByCategory.discontinued)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = ProductsByCategory.productName.name,
                                value = entity.getOptionalValue(ProductsByCategory.productName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = ProductsByCategory.quantityPerUnit.name,
                                value = entity.getOptionalValue(ProductsByCategory.quantityPerUnit)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = ProductsByCategory.unitsInStock.name,
                                value = entity.getOptionalValue(ProductsByCategory.unitsInStock)?.toString() ?: ""
                            )
                        )
                }
            }

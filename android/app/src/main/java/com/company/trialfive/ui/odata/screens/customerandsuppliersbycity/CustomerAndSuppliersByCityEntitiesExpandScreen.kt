...
val CustomerAndSuppliersByCityEntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        CustomerAndSuppliersByCityEntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        CustomerAndSuppliersByCityEntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        CustomerAndSuppliersByCityBlankScreen(viewModel)
                    }
                }
            }
        }
...
val CustomerAndSuppliersByCityBlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

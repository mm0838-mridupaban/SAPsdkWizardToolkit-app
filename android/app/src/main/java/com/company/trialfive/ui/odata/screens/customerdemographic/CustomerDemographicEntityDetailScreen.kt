package com.company.trialfive.ui.odata.screens.customerdemographic
...
import com.sap.cloud.android.odata.northwindentities.CustomerDemographic

val CustomerDemographicEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = CustomerDemographic.customerTypeID.name,
                                value = entity.getOptionalValue(CustomerDemographic.customerTypeID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = CustomerDemographic.customerDesc.name,
                                value = entity.getOptionalValue(CustomerDemographic.customerDesc)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, CustomerDemographic.customers as NavigationProperty)
                    }) {
                        Text("Customers", color = MaterialTheme.fioriHorizonAttributes.SapFioriColorT6)
                    }

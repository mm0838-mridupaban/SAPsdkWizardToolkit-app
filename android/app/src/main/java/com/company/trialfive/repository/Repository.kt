 ...
 * @param northwindEntities OData service
 ...
 */
class Repository(
        private val northwindEntities: NorthwindEntities,
    ...
    init {
        if (EntityMediaResource.isV4(northwindEntities.metadata.versionCode) && EntityMediaResource.hasMediaResources(entityType)) {
    ...
    suspend fun read(pageSize: Int = 40, page: Int = 0): Flow<List<EntityValue>> {
        return entitySet?.let{
            flow {
                ...
                try {
                    val result =  northwindEntities.executeQuery(dataQuery, httpHeaders).getEntityList().toList()
                    emit(result)
    ...
    suspend fun read(
        ...
        try {
            northwindEntities.loadProperty(navigationProperty, parent, dataQuery, httpHeaders)
    ...
    suspend fun suspendCreate(newEntity: EntityValue, media: StreamBase): SuspendOperationResult {
        if (newEntity.entityType.isMedia) {
            return try {
                northwindEntities.createMedia(newEntity, media)
    ...
    suspend fun suspendCreate(newEntity: EntityValue): SuspendOperationResult {
        ...
        return try {
            northwindEntities.createEntity(newEntity)
            SuspendOperationResult.SuspendOperationSuccess(
                newEntity
            )
        } catch (error: Exception) {
    ...
    suspend fun suspendCreateRelatedEntity(parent: EntityValue, newEntity: EntityValue, navPropName: String): SuspendOperationResult {
        ...
        return try {
            northwindEntities.createRelatedEntity(newEntity, parent, navProp)
            SuspendOperationResult.SuspendOperationSuccess(
                newEntity
            )
        } catch (error: Exception) {
    ...
    suspend fun suspendCreateRelatedEntity(parent: EntityValue, media: StreamBase, newEntity: EntityValue, navPropName: String): SuspendOperationResult {
        val navProp = parent.entityType.getProperty(navPropName)
        return try {
            northwindEntities.createRelatedMedia(newEntity, media, parent, navProp)
            SuspendOperationResult.SuspendOperationSuccess(
                newEntity
            )
        } catch (error: Exception) {
    ...
    suspend fun suspendUpdate(updateEntity: EntityValue): SuspendOperationResult {
        return try {
            northwindEntities.updateEntity(updateEntity)
            SuspendOperationResult.SuspendOperationSuccess(
                updateEntity
            )
        } catch (error: Exception) {
    ...
    suspend fun suspendDelete(deleteEntities: List<EntityValue>): SuspendOperationResult {
        ...
        return try {
            northwindEntities.applyChanges(deleteChangeSet)
            SuspendOperationResult.SuspendOperationSuccess()
        } catch (error: Exception) {

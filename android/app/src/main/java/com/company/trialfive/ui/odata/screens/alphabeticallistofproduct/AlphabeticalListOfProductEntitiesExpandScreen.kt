...
val AlphabeticalListOfProductEntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        AlphabeticalListOfProductEntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        AlphabeticalListOfProductEntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        AlphabeticalListOfProductBlankScreen(viewModel)
                    }
                }
            }
        }
...
val AlphabeticalListOfProductBlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

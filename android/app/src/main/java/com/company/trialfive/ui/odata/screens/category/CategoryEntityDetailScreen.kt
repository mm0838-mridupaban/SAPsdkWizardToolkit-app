package com.company.trialfive.ui.odata.screens.category
...
import com.sap.cloud.android.odata.northwindentities.Category

val CategoryEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Category.categoryID.name,
                                value = entity.getOptionalValue(Category.categoryID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Category.categoryName.name,
                                value = entity.getOptionalValue(Category.categoryName)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Category.description.name,
                                value = entity.getOptionalValue(Category.description)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Category.picture.name,
                                value = entity.getOptionalValue(Category.picture)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, Category.products as NavigationProperty)
                    }) {

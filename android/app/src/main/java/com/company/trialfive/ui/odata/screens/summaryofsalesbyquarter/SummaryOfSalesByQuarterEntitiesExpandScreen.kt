...
val SummaryOfSalesByQuarterEntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        SummaryOfSalesByQuarterEntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        SummaryOfSalesByQuarterEntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        SummaryOfSalesByQuarterBlankScreen(viewModel)
                    }
                }
            }
        }
...
val SummaryOfSalesByQuarterBlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

...
val SummaryOfSalesByYearEntitiesExpandScreen:
        @Composable (
            navigateToHome: () -> Unit,
            navigateUp: () -> Unit,
            ...
            Box(modifier = Modifier.weight(2f)) {
                when (uiState.entityOperationType) {
                    EntityOperationType.DETAIL -> {
                        SummaryOfSalesByYearEntityDetailScreen(onNavigateProperty, null, viewModel, true)
                    }

                    EntityOperationType.CREATE, EntityUpdateOperationType.UPDATE_FROM_LIST, EntityUpdateOperationType.UPDATE_FROM_DETAIL -> {
                        SummaryOfSalesByYearEntityEditScreen(null, viewModel, true)
                    }

                    else -> {
                        SummaryOfSalesByYearBlankScreen(viewModel)
                    }
                }
            }
        }
...
val SummaryOfSalesByYearBlankScreen:
        @Composable (
            viewModel: ODataViewModel,
        ) -> Unit =
    { viewModel ->

package com.company.trialfive.ui.odata.screens.summaryofsalesbyquarter
...
import com.sap.cloud.android.odata.northwindentities.SummaryOfSalesByQuarter

val SummaryOfSalesByQuarterEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SummaryOfSalesByQuarter.orderID.name,
                                value = entity.getOptionalValue(SummaryOfSalesByQuarter.orderID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SummaryOfSalesByQuarter.shippedDate.name,
                                value = entity.getOptionalValue(SummaryOfSalesByQuarter.shippedDate)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SummaryOfSalesByQuarter.subtotal.name,
                                value = entity.getOptionalValue(SummaryOfSalesByQuarter.subtotal)?.toString() ?: ""
                            )
                        )
                }
            }

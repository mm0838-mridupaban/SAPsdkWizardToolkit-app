package com.company.trialfive.ui.odata.screens.summaryofsalesbyyear
...
import com.sap.cloud.android.odata.northwindentities.SummaryOfSalesByYear

val SummaryOfSalesByYearEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SummaryOfSalesByYear.orderID.name,
                                value = entity.getOptionalValue(SummaryOfSalesByYear.orderID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SummaryOfSalesByYear.shippedDate.name,
                                value = entity.getOptionalValue(SummaryOfSalesByYear.shippedDate)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = SummaryOfSalesByYear.subtotal.name,
                                value = entity.getOptionalValue(SummaryOfSalesByYear.subtotal)?.toString() ?: ""
                            )
                        )
                }
            }

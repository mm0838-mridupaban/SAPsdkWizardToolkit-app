package com.company.trialfive.ui.odata.screens.territory
...
import com.sap.cloud.android.odata.northwindentities.Territory

val TerritoryEntityDetailScreen: @Composable (
                ...
                Column(
                    ...
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Territory.territoryID.name,
                                value = entity.getOptionalValue(Territory.territoryID)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Territory.territoryDescription.name,
                                value = entity.getOptionalValue(Territory.territoryDescription)?.toString() ?: ""
                            )
                        )
                    FioriKeyValueCell(
                        content = FioriKeyValueCellContent(
                                key = Territory.regionID.name,
                                value = entity.getOptionalValue(Territory.regionID)?.toString() ?: ""
                            )
                        )
                    TextButton(onClick = {
                        onNavigateProperty(entity, Territory.region as NavigationProperty)
                    }) {
                        Text("Region", color = MaterialTheme.fioriHorizonAttributes.SapFioriColorT6)
                    }

                    TextButton(onClick = {
                        onNavigateProperty(entity, Territory.employees as NavigationProperty)
                    }) {
                        Text("Employees", color = MaterialTheme.fioriHorizonAttributes.SapFioriColorT6)
                    }
